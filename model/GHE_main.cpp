...
json_data read_json()
{
    ...
    std::vector<double> lntts =  {-15.22015406,-15.08300806,-14.94586206,-14.80871605,-14.67157005,-14.53442405,-14.39727805,-14.26013205,-14.12298605,-13.98584005,-13.84869405,-13.71154804,-13.57440204,-13.43725604,-13.30011004,-13.16296404,-13.02581804,-12.88867204,-12.75152604,-12.61438004,-12.47723403,-12.34008803,-12.20294203,-12.06579603,  -11.92865003,
        -11.79150403,
        -11.65435803,
        -11.51721203,
        -11.38006602,
        -11.24292002,
        -11.10577402}; //length is 31
    std::vector<double> g_func = {-2.556919564,-2.483889186,-2.408186285,-2.329364731,-2.247153415,-2.161382726,-2.071953832,-1.978823254,-1.881993912,-1.781509211,-1.677448741,-1.569925155,-1.45908234,-1.34509531,-1.228172249,-1.108558858,-0.986544532,-0.862469138,-0.736728573,-0.609776928,-0.482123413,-0.354322868,-0.226959789,-0.100626916,  0.024099618,
        0.146685992,
        0.266664496,
        0.383652944,
        0.497367798,
        0.607629979,
        0.714363225}; //length is 31
...
std::tuple<std::vector<double>,std::vector<int>::size_type> g_expander(std::tuple <std::vector<double> ,std::vector<double>, std::vector<double>> test_data, double ts){
    ...
    for (double q_lntts_temp: q_lntts){
        ...
        // In this case, the g-function must be found by extrapolation.
            ...
            output_g_values[j] = ((q_lntts_temp - lntts[Npairs]) / (lntts[Npairs - 1] - lntts[Npairs])) * (g_func[Npairs - 1] - g_func[Npairs]) + g_func[Npairs];}
        ...
        while (Low <= High) {
            ...
            if (lntts[Mid] < q_lntts_temp) {
                ...
                if (lntts[Mid] > q_lntts_temp) { High = Mid - 1;}
                else {Found = true;
                    break;}}}
        // q_lntts_temp is identical to one of the lntts array elements return gFuncVal
        // the gFuncVal after applying the correction
        if (Found) {
            output_g_values[j] = g_func[Mid];}

        // q_lntts_temp is in between any of the two lntts array elements find the
        // g-function value by interpolation and apply the correction and return gFuncVal
        else {
            if (lntts[Mid] < q_lntts_temp) ++Mid;
...
double summation(int n, std::vector<double> q_load, std::vector<double> g_data){
    ...
    int j = n;
    while ( i <= n ){
        if (n==0){q_delta = q_load[i]-0;}
        else {q_delta = q_load[i]-q_load[i-1];}
        // eqn 1.11
...
output_data simple_GHE(double mdot){
    ...
    double alpha_s = pow(10,-7); //TODO: not sure whether to use this estimate for alpha_s or the below equation, which was copied from the python code.
