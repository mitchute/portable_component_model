...
//    int i = 0;
//    //TODO: determine i as index for referencing in vectors. For loop needed here.
//    // start for loop
//    x1 = lntts[i];
//    x2 = lntts[i+1];
//    y1 = g_func[i];
...
//-------------------------------
    ...
    //TODO: calculate lntts from the t in the q data. The match that to the lnnts in JSON to get the g function data for that q point. See teams message (DONE). Also ask Matt if short time periods will have errors, wierd numbers coming through with dummy data i.e. Tout = 8000 when Tin = 20. Possibly also error in dummy input data.
    
    
    /* Notes from Teams Meeting:
    g functions are a mathematical tool for deriving these models. It is the non-dimensional temp rise of the borehole divided by the non-dimensional time.
    close to zero time units are min, far from zero units are 100 of years. He will send me some values for a single borehole for testing purposes.  */
}
